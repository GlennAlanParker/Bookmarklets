(() => {
    try {
        ...
        function pickLargestFromSrcset(srcset) {
            ...
            } catch { return srcset.split(',').slice(-1)[0].trim().split(/\s+/)[0]; }
        }

        // --- Extract original/unproxied URL ---
        ...
        // --- Fetch actual server dimensions ---
        ...
        // --- Collect images including galleries ---
        const imgs = [...d.querySelectorAll("img")].filter(e => {
        ...
        for (const img of imgs) {
            ...
            items.push({
                name,
                dim: `${img.width}×${img.height} rendered`, // will be updated with full size later
                size: "Fetching...",
                alt: img.alt || "None",
                caption,
                url: native,
        ...
        Object.assign(o.style,{position:"fixed",top:"10px",right:"0",width:"520px",height:"240px",maxHeight:"95vh",display:"flex",flexDirection:"column",background:"#f8f9fa",font:"12px Arial, sans-serif",zIndex:2147483647,border:"1px solid #ccc",borderRadius:"10px",boxShadow:"0 4px 12px rgba(0,0,0,0.15)",overflow:"hidden",left:"auto"});

        const headerH=56, footerH=14;

        function mkbar(pos){
            const b=d.createElement("div");
            Object.assign(b.style,{height: pos==="top"?headerH+"px":footerH+"px",display:"flex",alignItems:"center",justifyContent: pos==="top"?"space-between":"flex-end",padding: pos==="top"?"6px 10px":"2px 8px",background:"#34495e",color:"#fff",fontWeight:700,cursor:"grab",userSelect:"none"});
            if(pos==="top"){
                const title=d.createElement("h1"); title.textContent="Image Data"; Object.assign(title.style,{margin:0,color:"#fff",fontSize:"16px",textAlign:"left"}); b.appendChild(title);
                ...
                const toggleBtn=d.createElement("button"); toggleBtn.textContent="🔢"; toggleBtn.title="Toggle Number Badges"; Object.assign(toggleBtn.style,{border:"none",background:"transparent",fontSize:"14px",cursor:"pointer",color:"#fff"}); toggleGroup.appendChild(toggleBtn);
        ...
        scrollTopBtn.addEventListener("click",()=>{txt.scrollTo({top:0,behavior:"smooth"});}); txt.appendChild(scrollTopBtn);
        txt.addEventListener("scroll",()=>{scrollTopBtn.style.display=txt.scrollTop>20?"flex":"none";});
        const checkScroll=()=>{scrollTopBtn.style.display=txt.scrollTop>20?"flex":"none";}; txt.addEventListener("scroll",checkScroll); setTimeout(checkScroll,100);
...
${it.caption?`<div><strong>Caption:</strong> ${it.caption}</div>`:""}`;
        ...
        // --- Fetch full image dimensions & size ---
        (async ()=>{
        ...
        ["n","s","e","w","ne","nw","se","sw"].forEach(dir=>{
            ...
            h.addEventListener("pointerdown",e=>{
                ...
                let startX=e.clientX,startY=e.clientY,r=o.getBoundingClientRect(),startW=r.width,startH=r.height,startL=r.left,startT=r.top; const minW=200,minH=140;
                const onMove=me=>{
