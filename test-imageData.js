...
// ---------------------------------------------------------------------
    // Robust z-index enforcement:
    // 1) inject stylesheet that lowers our badges and raises common lightbox classes
    // 2) add MutationObserver to catch overlays/lightboxes added later
    // ---------------------------------------------------------------------
    const injectedStyle = d.createElement('style');
    injectedStyle.id = "img-data-zindex-fix";
    injectedStyle.textContent = `
        /* mark badges we created and ensure they are below overlays */
        .img-data-badge, [data-img-data-badge="1"] {
            z-index: 1000 !important;
        }

        /* common lightbox/overlay containers — raise them above badges */
        .lightbox,
        .jquery-lightbox,
        .fancybox-container,
        .featherlight,
        ...
        /* safety: anything directly appended to body with role="dialog" or data-lightbox
           also get a boost — covers custom plugins */
        body > [role="dialog"],
        body > [data-lightbox],
        body > .lightbox,
        body > .modal {
    ...
    window._imgData.__injectedStyle = injectedStyle;

    // Observer to bump up z-index of overlays added after this script runs
            ...
            if(m.addedNodes && m.addedNodes.length){
                m.addedNodes.forEach(node => {
                    try {
                        ...
                        // if node looks like an overlay/lightbox by class or attribute, boost it
                        const lookLikeOverlay = (
                            node.matches && (
                                node.matches('.lightbox, .jquery-lightbox, .fancybox-container, .featherlight, .mfp-wrap, .lg-backdrop, .lg-outer, .lightboxOverlay, #lightbox, .modal, [data-lightbox], [role="dialog"]')
                            )
                        );
                        if(lookLikeOverlay){
                            try { node.style.zIndex = "200000"; } catch(e){}
                            // ensure children also get increased z-index where inline styles are present
                            node.querySelectorAll && node.querySelectorAll('*').forEach(el=>{
                                try { el.style && (el.style.zIndex = el.style.zIndex || "200001"); } catch(e){}
                            });
                        }
                        // also if some plugin creates inline style z-index values extremely large on our badges, clamp them:
                        if(node.querySelectorAll){
                            const foundBadges = node.querySelectorAll ? node.querySelectorAll('.img-data-badge,[data-img-data-badge="1"]') : [];
                            foundBadges.forEach(b => {
                                try { b.style.zIndex = "1000"; } catch(e){}
                            });
                        }
                    } catch(e){}
                });
    ...
    window._imgData.__zWatchObserver = observer;

    // extra safety: also clamp any existing badges that might have inline z-index overrides
    try {
        const existingBadges = document.querySelectorAll('.img-data-badge,[data-img-data-badge="1"]');
        existingBadges.forEach(b => { try{ b.style.zIndex = "1000"; }catch(e){} });
    } catch(e){}

    // ---------------------------------------------------------------------
    // End robust z-index enforcement
    // ---------------------------------------------------------------------
