javascript:(function(){
  ...
  // Build fixed table header wrapper
  var theadWrap=d.createElement("div"); theadWrap.id="theadWrap";
  var thTable=d.createElement("table"); var thead=d.createElement("thead"); var tr=d.createElement("tr");
  ["#","Link","HTML"].forEach(function(h){ var th=d.createElement("th"); th.textContent=h; tr.appendChild(th); });
  thead.appendChild(tr); thTable.appendChild(thead); theadWrap.appendChild(thTable); d.body.appendChild(theadWrap);

  // Build main table
  var table=d.createElement("table"); table.id="tbl"; table.appendChild(d.createElement("tbody")); d.body.appendChild(table);

  // Back-to-top
  var btt=d.createElement("div"); btt.id="backToTop"; btt.textContent="↑"; d.body.appendChild(btt);

  // Add CSS
  var style=d.createElement("style"); style.textContent="body{font-family:Arial;margin:0;text-align:center}"+
  ...
  j.onload=function(){
    ...
    function o(e,t){if(!t||!t.trim())return;var n=new RegExp(t.trim().split(/\s+/).map(r).join("|"),"gi");
      !function r(o){3===o.nodeType?(function(){var r=o.parentNode,i=o.textContent;if(n.test(i)){var a=document.createElement("span");a.innerHTML=i.replace(n,e=>'<span class="highlight">'+e+"</span>');r.replaceChild(a,o)}}()):1===o.nodeType&&o.childNodes.length&&Array.from(o.childNodes).forEach(r)}(e);
    }
    ...
    function renderTable(data,search){
      ...
      tbody.querySelectorAll("td.linkcol").forEach(function(td){o(td,search)});
      tbody.querySelectorAll("td.markupcol").forEach(function(td){td.innerHTML=a(td.innerHTML,search)});
      updateCount();
    }
    ...
    $("#sort",d).on("change",function(){
      var val=this.value;
      if(val==="text-asc") e.sort(function(a,b){return a.text.localeCompare(b.text)});
      if(val==="text-desc") e.sort(function(a,b){return b.text.localeCompare(a.text)});
      if(val==="url-asc") e.sort(function(a,b){return a.href.localeCompare(b.href)});
      if(val==="url-desc") e.sort(function(a,b){return b.href.localeCompare(a.href)});
    ...
    $("#filter",d).on("change",function(){
      ...
      if(val==="internal") e=l.filter(function(a){return a.href.includes(h)});
      if(val==="external") e=l.filter(function(a){return !a.href.includes(h)});
      renderTable(e,$("#search",d).val());
    });
